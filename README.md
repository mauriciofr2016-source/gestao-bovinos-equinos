...
## ✅ Atualização v5 — Trato Multiingrediente + Estoque Automático

### Novidades implementadas

#### Módulo de Trato
- **Multiingrediente**: Adicione quantos ingredientes quiser em um único trato.
- **Campos por ingrediente**: nome (dropdown de insumos), quantidade, unidade (kg, g, L, mL, saco, unid), custo unitário e subtotal calculado automaticamente.
- **Custo real do estoque**: O custo unitário é preenchido automaticamente com o `custo_kg` do estoque cadastrado. Caso o insumo não exista no estoque, usa valor estimado de fallback com aviso visual.
- **Verificação de saldo**: Alerta visual antes de salvar quando o saldo em estoque é insuficiente para a quantidade informada.
- **Observações**: Campo livre para anotações no trato.
- **Anti-duplo-clique**: Botão travado durante o salvamento, com texto "Salvando..." e desbloqueio automático.
- **Mensagens claras**: Toast de sucesso com custo total, toast de erro em falhas, aviso de saldo insuficiente.

#### Débito automático de estoque
Ao salvar o trato, o sistema debita automaticamente a quantidade de cada ingrediente do saldo em estoque correspondente. Unidades `g` são convertidas para `kg` (÷1000). Se o insumo não existe no estoque, o trato é registrado normalmente sem debitar (nenhuma exceção silenciosa).

#### Histórico de tratos
- Exibe cada ingrediente com nome, quantidade, unidade, custo unitário e subtotal.
- Exibe observações se preenchidas.
- Mantém os últimos 20 registros visíveis na aba Trato.
- Na ficha individual do animal, os tratos também exibem ingredientes detalhados.
...
users/{uid}/tratos/{id}
  ...
  - desc: string (descrição resumida)
  - donoUid: string
  - _ts: timestamp
  - itens: Array<{
      insumo: string           // chave: racao, silagem, feno, sal, suplemento, outro
      nome: string             // label legível: "Ração", "Silagem"...
      qtd: number              // quantidade
      unidade: string          // kg, g, L, mL, saco, unid
      custoUnit: number        // custo por unidade (R$)
      subtotal: number         // qtd × custoUnit
    }>
```

### Compatibilidade retroativa
Tratos antigos (sem campo `nome`, `unidade`, `custoUnit`) são exibidos normalmente usando o campo `desc` ou `insumo` como fallback. Nenhum dado existente é apagado ou migrado.
